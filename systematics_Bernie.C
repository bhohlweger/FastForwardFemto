...
// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
int systematics_Bernie(const char* file,const char *output="systematics_output")
{
  ...
  const Int_t NTasks = 19;//Number of different total variations (including default)
  ...
  {
    if(taskNumber == 0) outputfile += "MBResults";
    else if(taskNumber == 1) outputfile = "MBResults1";
    else if(taskNumber == 2) outputfile = "MBResults2";
    else if(taskNumber == 3) outputfile = "MBResults3";
    else if(taskNumber == 4) outputfile = "MBResults4";
    else if(taskNumber == 5) outputfile = "MBResults5";
    else if(taskNumber == 6) outputfile = "MBResults6";
    else if(taskNumber == 7) outputfile = "MBResults7";
    else if(taskNumber == 8) outputfile = "MBResults8";
    else if(taskNumber == 9) outputfile = "MBResults9";
    else if(taskNumber == 10) outputfile = "MBResults10";
    else if(taskNumber == 11) outputfile = "MBResults11";
    else if(taskNumber == 12) outputfile = "MBResults12";
    else if(taskNumber == 13) outputfile = "MBResults13";
    else if(taskNumber == 14) outputfile = "MBResults14";
    else if(taskNumber == 15) outputfile = "MBResults15";
    else if(taskNumber == 16) outputfile = "MBResults16";
    else if(taskNumber == 17) outputfile = "MBResults17";
    else if(taskNumber == 18) outputfile = "MBResults18";

    TDirectoryFile *dirResults=(TDirectoryFile*)(_file0->FindObjectAny(outputfile.Data()));
    dirResults->GetObject(outputfile.Data(), listTP[taskNumber]);
  ...
  std::vector<const char*> protonCut(9);
  ...
  protonCut[7] = "TPC cluster down";
  protonCut[8] = "TPC cluster up";
  ...
  double sysErrorPPAPAP_TPCclusterUp[NkstarBinsPPAPAP];
  double sysErrorPPAPAP_TPCclusterDown[NkstarBinsPPAPAP];
  ...
  for(int kstarBin = 0; kstarBin < NkstarBinsPPAPAP; kstarBin++) {
    ...
    sysErrorPPAPAP_TPCclusterUp[kstarBin] = 0;
    sysErrorPPAPAP_TPCclusterDown[kstarBin] = 0;
...
//    for(int protonVariation = 0; protonVariation < NTasksProtonVariations; protonVariation++) {
  ...
  for(int protonVariation = 0; protonVariation < NTasksProtonVariations; protonVariation++)
  {
    ...
    for(int kstarBin = 0; kstarBin < NkstarBinsPPAPAP; kstarBin++)
    {
      ...
      {
        sysErrorPPAPAP_TPCclusterDown[kstarBin] = 0.;
        sysErrorPPAPAP_TPCclusterDown[kstarBin] = fabs(1. - pow(SysError,-1)) * hCFPPAPAP[0]->GetBinContent(kstarBin+1);
        hCFPPAPAP_errBudget[protonVariation]->SetBinContent(kstarBin+1,fabs(1. - pow(SysError,-1)));
      }
      else if(protonVariation == 8)//TPC cluster
      {
        sysErrorPPAPAP_TPCclusterUp[kstarBin] = 0.;
        sysErrorPPAPAP_TPCclusterUp[kstarBin] = fabs(1. - pow(SysError,-1)) * hCFPPAPAP[0]->GetBinContent(kstarBin+1);
        hCFPPAPAP_errBudget[protonVariation]->SetBinContent(kstarBin+1,fabs(1. - pow(SysError,-1)));
      }
    }
  }
  ...
  std::vector<const char*> v0Cut(17);
  ...
  v0Cut[9] = "V0 #it{p}_{T} down";
  v0Cut[10] = "V0 #it{p}_{T} up";
  v0Cut[11] = "V0 CPA up";
  v0Cut[12] = "V0 n#sigma down";
  v0Cut[13] = "V0 TPC ncls 80";
  v0Cut[14] = "V0 #eta up";
  v0Cut[15] = "V0 #eta down";
  v0Cut[16] = "V0 d_{track} down";
  v0Cut[17] = "V0 d_{Track, PV} up";
  ...
  for(int kstarBin = 0; kstarBin < NkstarBinsPPAPAP; kstarBin++)
  {
    ...
    double sysErrorAverageTPCclus = 0.5*(sysErrorPPAPAP_TPCclusterUp[kstarBin] + sysErrorPPAPAP_TPCclusterDown[kstarBin]);

    //Add them up quadratically:

    sysErrorTotalPPAPAP[kstarBin] = pow(sysErrorAveragePt,2.) + pow(sysErrorAverageEta,2.) + pow(sysErrorAverageNSigma,2.) + pow(sysErrorPPAPAP_FilterBit[kstarBin],2.) + pow(sysErrorAverageTPCclus,2.);
  ...
  for(int kstarBin = 0; kstarBin < NkstarBinsPLAPAL; kstarBin++)
  {
    ...
    double sysErrorAverageTPCclus = 0.5*(sysErrorPLAPAL_TPCclusterUp[kstarBin] + sysErrorPLAPAL_TPCclusterDown[kstarBin]);
