...
void fit_forL_ppHM(){
  ...
  f0->SetParameters(1.07608e+00,-1.26658e-04,9.03790e-01,-5.29577e-02);
  f0->SetLineStyle(2);
  f0->SetLineColor(3);
  model->Fit(f0,"R");
  ...
  TFile *_file1 = TFile::Open("ppHM_13TeV_CFOutput_pXi_Rebin_5.root");
  TH1F*  data = (TH1F*) _file1->Get("hCkTotNormWeight");
  ...
  f1->SetParameter(4,.2);
  f1->SetParameter(5,89.);
//  f1->SetParLimits(5,60,70);
  f1->SetParameter(6,4.);
  ...
  data->GetYaxis()->SetRangeUser(0.7,2.5);
  //dataFAKE->Draw("same");
  dataAGAIN->Draw("same");
  data->Draw("same");
  f0->Draw("same");
  ...
  leg2->AddEntry(data,"ALICE p-p 13TeV");
